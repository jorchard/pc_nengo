...
def train_network(sim, learn_until):
...
def add_inference_inputs(X, Y, x_dict, y_dict, timeout, stab_time, learn_until):
    ...
    timeout             Num, the amount of simulation time to spend before reaching equilibrium.
    ...
    #input validation
    if timeout == 0:
        raise ValueError("timeout cannot be zero or only one instance will be added to the dictionary, overriding previous inputs.")
    #size of the dataset
    if len(X.shape) == 1:
        num_points = X.shape[0]
        num_features = 1
    ...
    for i in indices:
        if num_features == 1:
            x_dict[learn_until] = np.zeros_like(X[i])
            x_dict[learn_until + stab_time] = X[i]
        else:
            x_dict[learn_until] = np.zeros_like(X[i,:])
            x_dict[learn_until + stab_time] = X[i,:]
        if target_dimensions == 1:
            y_dict[learn_until] = np.zeros_like(Y[i])
            y_dict[learn_until + stab_time] = Y[i]
        else:
            y_dict[learn_until] = np.zeros_like(Y[i,:])
            y_dict[learn_until + stab_time] = Y[i,:]
        
        learn_until += timeout + stab_time #increment the time step
    
    return Y.shape[0]


def test_network(sim, pc_net, num_predictions, timeout, stab_time, output_probe):
    """
    ...
    for i in range(num_predictions):
        sim.run(stab_time)
        sim.run(timeout) #reach equilibrium
...
if __name__ == "__main__":
    ...
    preds = test_network(sim, PC_net, num_preds, inf_time, stab_time, val_probes[-1])
